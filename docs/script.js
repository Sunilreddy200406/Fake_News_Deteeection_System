...
exportCsvLink.href = apiUrl("/history/export");
...
checkBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    const confidenceText = data.confidence ? `${(data.confidence * 100).toFixed(2)}%` : "N/A";
    ...
    showResult("Cannot reach backend API. Update docs/config.js API_BASE_URL.", "error");
  }
});
...
prevPageBtn.addEventListener("click", async () => {
  if (currentPage > 1) await loadHistory(currentPage - 1);
});
...
loadHistory(1).catch(() => {
  showResult("Cannot load history. Check backend URL in docs/config.js.", "error");
});
